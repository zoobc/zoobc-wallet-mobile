...
// import { NrsBridge, converters } from "@keyhub/keyhub-vault-nxt"
import wordlistEnEff from "diceware-wordlist-en-eff"
import dicewareGen from "../utils/diceware"
import { normalizeMsg } from "../utils/webworker"
import { callOnStore } from "../utils/indexeddb"
// import {
//   safeObj,
//   encrypt,
//   decrypt,
...
import { safeObj }  from "../utils/crypto"
...
export const KEYHUB = () => {
  const log = (...args) => {
    ...
    if (typeof console !== "undefined" && console.log)
      console.log(
        `${name}:`,
        ...args.map(arg => JSON.stringify(arg, null, "  "))
      );
  };
  ...
  //   const digestKeyUint8 = Uint8Array.from(
  //     converters.stringToByteArray(`${networkName} seed`)
  //   );
  ...
  //       const masterPhraseUint8 = new Uint8Array(
  //         digestBuffer,
  //         0,
  ...
  //       const masterPhraseHex = converters.byteArrayToHexString(
  //         Array.from(masterPhraseUint8)
  //       );
  ...
  //       txData[`${key}NQT`] = NRS.calculatePricePerWholeQNT(
  //         NRS.convertToNQT(val),
  //         decimals
  //       );
  ...
  //       converters.stringToByteArray(`${networkName} seed`)
  //     );
  //     const passphraseUint8 = Uint8Array.from(
  //       converters.stringToByteArray(passphrase)
  //     );
  //     // Hash the passphrase to get the nxt masterPhrase
  //     return digestKeyed(digestKeyUint8, passphraseUint8, {
  //       name: "SHA-512"
  //     }).then(digestBuffer => {
  //       // Note: Non-truncated SHA is vulnerable to length extension attack (e.g. sha256 & sha512)
  //       // We use a longer hash here and truncate to desired AES keySize (e.g. 256)
  ...
  //       throw new Error("address / entryId is not a string");
  ...
  //               hasPinProtection: !!(
  //                 entry.masterPhrase && entry.masterPhrase.deriveAlgo
  //               ),
  ...
  //       throw new Error("passphrase is not a string");
  ...
  //     if (typeof passphrase !== "string")
  //       throw new Error("passphrase is not a string");
  //     if (typeof masterPin !== "string")
  //       throw new Error("masterPin is not a string");

  //     const passphraseUint8 = Uint8Array.from(
  //       converters.stringToByteArray(passphrase)
  //     );
  //     const masterPinUint8 = Uint8Array.from(
  //       converters.stringToByteArray(masterPin)
  //     );
  ...
  //       throw new Error("txData is not an object");
  ...
  //             reject(
  //               new Error("key masterPhrase is not stored in this browser")
  //             );
  //             return;
  //           }
  ...
  //               .catch(err => {
  //                 if (err.type === "OperationError")
  //                   throw Error("PIN is incorrect");
  //                 throw err;
  //               })
  ...
  //                 if (!defaultCryptoKey) {
  //                   reject(
  //                     new Error("vault masterKey is missing in this browser")
  //                   );
  //                   return;
  //                 }
  ...
  //       throw new Error("message is not a hex string");
    ...
    self.onmessage = event => {
      ...
      if (!(name in methods)) {
        resultPort.postMessage(
          normalizeMsg({ error: new Error("invalid method name") })
        );
      }
